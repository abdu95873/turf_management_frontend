...
import { filterFutureSlots, formatTimeRange, isSlotPast } from "../../lib/slotTime";
...
export default function VenueLandingPage() {
  ...
  const slotsQuery = useQuery({
    queryKey: ["venue-slots", resourceId, selectedDate],
  ...
  const bookableSlots = useMemo(() => {
    ...
    return future.filter((slot) => slot.status === "available");
  }, [slotsQuery.data, selectedDate, nowTick]);
  const selectedSlot = bookableSlots.find((slot) => slot._id === selectedSlotId) ?? null;
  const bookingAmount = selectedSlot ? resource?.pricePerHour ?? 0 : 0;
  ...
  if (detailsQuery.isError || !resource) {
    return (
      <main className="mx-auto max-w-[1180px] px-4 py-10 md:px-6">
        <Link to="/" className="inline-flex items-center gap-2 text-sm font-medium text-ds-primary hover:underline">
    ...
    <main className="pb-10 font-sans text-ds-secondary">
      <section className="relative left-1/2 right-1/2 w-screen ml-[-50vw] mr-[-50vw]">
        <div className="relative h-[280px] overflow-hidden md:h-[360px]">
          ...
          <div className="absolute bottom-0 left-0 right-0 mx-auto max-w-[1180px] px-4 pb-8 md:px-6">
            ...
            <span className="mb-2 inline-flex rounded-full bg-ds-primary/90 px-3 py-1 text-xs font-semibold uppercase text-white">
      ...
      <div className="mx-auto mt-8 max-w-[1180px] space-y-8 px-4 md:px-6">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {packages.map((pkg) => (
                <article key={pkg._id} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                  <p className="text-xs font-semibold uppercase text-ds-primary">{pkg.sportType}</p>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-600">Pick a date, select a slot, then pay with SSLCommerz.</p>
          ...
          <div className="mt-4 max-w-xs">
            ...
            <input
              type="date"
              className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-ds-primary focus:ring-2 focus:ring-ds-primary/20"
          ...
          {!slotsQuery.isLoading && bookableSlots.length ? (
            <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              {bookableSlots.map((slot) => (
                <button
                  ...
                  className={`rounded-xl border px-4 py-3 text-left transition ${
                    selectedSlotId === slot._id
                      ? "border-ds-primary bg-ds-primary/10 ring-2 ring-ds-primary/30"
                ...
                >
                  ...
                  <p className="mt-1 text-xs font-semibold text-emerald-700">Available</p>
                </button>
              ))}
            </div>
          ) : null}
